...
//The type of the termination criteria:
    ...
    Parametrizar(max_depth,min_sample_count,regression_accuracy,use_surrogates,max_categories,cv_folds,use_1se_rule,truncate_pruned_tree,priors,calc_var_importance,native_vars);
    nombre=Nombre;
    tipoClasificador=RTREES;
}

MLT::Clasificador_RTrees::~Clasificador_RTrees(){}

int MLT::Clasificador_RTrees::Parametrizar(int max_depth, int min_sample_count, float regression_accuracy, bool use_surrogates, int max_categories, int cv_folds, bool use_1se_rule, bool truncate_pruned_tree, Mat priors, bool calc_var_importance, int native_vars){
    TREES=ml::RTrees::create();
    ...
    TREES->setCalculateVarImportance(calc_var_importance);
    TREES->setActiveVarCount(native_vars);
    TREES->setTermCriteria(cvTermCriteria( CV_TERMCRIT_ITER+CV_TERMCRIT_EPS, 50, 0.1 ));
    return 0;
}

int MLT::Clasificador_RTrees::Autotrain(vector<Mat> Data, vector<float> Labels, Dimensionalidad::Reducciones reduc, Generacion::Info_Datos info, bool save){
    if(Data.size()==0){
...
void MLT::Clasificador_RTrees::Entrenamiento(Mat trainingDataMat, Mat labelsMat){
...
float MLT::Clasificador_RTrees::Clasificacion(Mat Data){
...
int MLT::Clasificador_RTrees::SaveData(){
...
int MLT::Clasificador_RTrees::ReadData(){
    string g="../Data/Configuracion/"+nombre+"/RTREES2.xml";
    ...
    g="../Data/Configuracion/"+nombre+"/RTREES.xml";
//    TREES->load(g.c_str());
    TREES = ml::StatModel::load<ml::RTrees>(g.c_str());
    return 0;
}
