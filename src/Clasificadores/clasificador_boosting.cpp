...
int MLT::Clasificador_Boosting::Parametrizar(int boost_type, int weak_count, double weight_trim_rate, int max_depth, bool use_surrogates, Mat priors){
    ...
    BOOST->setWeightTrimRate(weight_trim_rate);
...
int MLT::Clasificador_Boosting::Autotrain(vector<Mat> Data, vector<float> Labels, Dimensionalidad::Reducciones reduc, Generacion::Info_Datos info, bool save){
    int e=0;
    if(Data.size()==0){
...
void MLT::Clasificador_Boosting::Entrenamiento(Mat trainingDataMat, Mat labelsMat){
...
int MLT::Clasificador_Boosting::Read_Data(){
    ...
    g="../Data/Configuracion/"+nombre+"/BOOST.xml";
//    BOOST->load(g.c_str());
    BOOST = ml::StatModel::load<ml::Boost>(g.c_str());
    return 0;
}
