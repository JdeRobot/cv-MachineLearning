...
MLT::Clasificador_KNN::Clasificador_KNN(string Nombre, int k, bool regression){
    Parametrizar(k,regression);
    nombre=Nombre;
    tipoClasificador=KNN;
}

MLT::Clasificador_KNN::~Clasificador_KNN(){}

int MLT::Clasificador_KNN::Parametrizar(int k, bool regression){
    KN=ml::KNearest::create();
    k_neig=k;
    reg=regression;
    return 0;
}

int MLT::Clasificador_KNN::Autotrain(vector<Mat> Data, vector<float> Labels, Dimensionalidad::Reducciones reduc, Generacion::Info_Datos info, bool save){
    if(Data.size()==0){
...
void MLT::Clasificador_KNN::Entrenamiento(Mat trainingDataMat, Mat labelsMat){
    trainingDataMat.convertTo(trainingDataMat,CV_32FC1);
    labelsMat.convertTo(labelsMat,CV_32S);
    trainingDataMat.copyTo(K.Datos);
    labelsMat.copyTo(K.Labels);
    KN->train(trainingDataMat, ml::ROW_SAMPLE, labelsMat);
}

float MLT::Clasificador_KNN::Clasificacion(Mat Data){
    Data.convertTo(Data,CV_32FC1);
    float response=0;
    if(Data.cols==(ventanaX*ventanaY) || Data.cols==reduccion.tam_reduc){
        Mat results;
        response=KN->findNearest(Data,k_neig,results);
    }
    return response;
}


int MLT::Clasificador_KNN::SaveData(){
    ...
    if(archivo_w.isOpened()){
        ...
        archivo_w<<"k_neig"<<k_neig;
        archivo_w<<"regression"<<reg;
    }
    else
        return 1;
    archivo_w.release();
...
int MLT::Clasificador_KNN::ReadData(){
    string g="../Data/Configuracion/"+nombre+"/KNN2.xml";
    ...
    if(archivo_r.isOpened()){
        ...
        archivo_r["k_neig"]>>k_neig;
        archivo_r["regression"]>>reg;
    }
    else
        return 1;
    archivo_r.release();
    ...
    if(archivo_r2.isOpened()){
        archivo_r2["Datos"]>>trainingDataMat;
        archivo_r2["Labels"]>>labelsMat;
    }
    else
        return 1;
    Entrenamiento(trainingDataMat, labelsMat);
    archivo_r2.release();
    return 0;
